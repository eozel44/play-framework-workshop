...
trait Tables {
  ...
  implicit def GetResultTasksRow(implicit e0: GR[Int], e1: GR[String]): GR[TasksRow] = GR{
    prs => import prs._
  ...
  class Tasks(_tableTag: Tag) extends profile.api.Table[TasksRow](_tableTag, Some("playwithscala"), "tasks") {
    ...
    def ? = ((Rep.Some(taskId), Rep.Some(userId), Rep.Some(text))).shaped.<>({r=>import r._; _1.map(_=> TasksRow.tupled((_1.get, _2.get, _3.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
