...
trait Tables {
  ...
  case class ItemsRow(itemId: Long, userId: Int, text: String)
  /** GetResult implicit for fetching ItemsRow objects using plain SQL queries */
  implicit def GetResultItemsRow(implicit e0: GR[Long], e1: GR[Int], e2: GR[String]): GR[ItemsRow] = GR{
    prs => import prs._
  ...
  class Items(_tableTag: Tag) extends profile.api.Table[ItemsRow](_tableTag, Some("playwithscala"), "items") {
